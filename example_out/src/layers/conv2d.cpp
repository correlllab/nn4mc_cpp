...
char activation)
{
	...
	layer.output_shape[0] = layer.input_shape_0 - layer.kernel_shape_0 + 1;
	layer.output_shape[1] = layer.input_shape_1 - layer.kernel_shape_1 + 1;
...
void fwdConv2D(struct Conv2D L, float* input)
{
				...
				for(int kernel_position_x = 0; kernel_position_x < L.kernel_shape[0]; kernel_position_x++)
				{
					...
					{				
                        ...
                        if (ii >= 0 && ii < i && jj >= 0 && jj < j){
                            h[output_idx] += *(W + L.weight_shape[3]*L.weight_shape[2]*L.weight_shape[1]*kernel_position_x + L.weight_shape[2]*L.weight_shape[1]* kernel_position_y + L.weight_shape[1]*f) * (input + L.input_shape[1]*L.input_shape[2]*ii + L.input_shape[2] * jj);
                       }
                   } 
